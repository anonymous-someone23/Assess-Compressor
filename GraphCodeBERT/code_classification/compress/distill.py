...
def train(args, model, train_dataloader, eval_dataloader):
    ...
    for epoch in range(args.epochs):
        ...
        logger.info("Train Loss: {0}, Val Acc: {1}, Val Precision: {2}, Val Recall: {3}, Val F1: {4}".format(
            train_loss/tr_num, dev_results["eval_acc"], dev_results["eval_precision"], dev_results["eval_recall"], dev_results["eval_f1"]))


def evaluate(model, eval_dataloader):
    # tokenizer = eval_dataloader.dataset.getTokenizer()
    model.eval()
    predict_all = []
    labels_all = []
    ...
    with torch.no_grad():
        ...
        for batch in bar:
            ...
            # print(np.sum(texts.detach().cpu().numpy() == tokenizer.token_to_id("<pad>"), axis=-1))
            # break

            prob = model(texts)
            prob = F.softmax(prob).cpu().numpy()
            predict_all.append(prob.argmax(axis=1))
    ...
    precision = precision_score(labels_all, preds, average='macro')
